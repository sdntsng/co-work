...
export async function deleteLead(leadId: string): Promise<void> {
  const response = await fetch(`${API_BASE}/api/leads/${leadId}`, { method: 'DELETE' });
  return handleResponse(response);
}
...
export async function updateOpportunityStage(oppId: string, stage: string): Promise<void> {
  const response = await fetch(`${API_BASE}/api/opportunities/${oppId}/stage`, {
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' },
...
export async function deleteOpportunity(oppId: string): Promise<void> {
  const response = await fetch(`${API_BASE}/api/opportunities/${oppId}`, { method: 'DELETE' });
  return handleResponse(response);
}
