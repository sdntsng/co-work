...
# Global CRM manager (initialized on first request)
_crm_manager: Optional[CRMManager] = None


def get_crm(sheet_name: str = "Sales Pipeline 2026") -> CRMManager:
    """Get or create CRM manager instance."""
    global _crm_manager
    if _crm_manager is None:
        gc, _ = authenticate()
        _crm_manager = CRMManager(SheetManager(gc), sheet_name)
    return _crm_manager
...
def update_opportunity_stage(opp_id: str, data: StageUpdate):
